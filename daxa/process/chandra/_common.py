...
#         from daxa.process.chandra import parse_oif
#
...
#
#         # Seeing if any of the Chandra missions (there should probably only ever be one) in the archive have had
#         #  any processing done yet
#         chandra_miss = [mission for mission in obs_archive if mission.name in ALLOWED_CHANDRA_MISSIONS]
#         for miss in chandra_miss:
#             # Getting the process_logs for each mission - use the attribute to avoid triggering the error about how
#             #  we haven't done any processing yet - if this is the first bit of processing done on the archive
