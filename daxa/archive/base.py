...
class Archive:
        ...
        for mn in einfo_info:
            ...
            for rel_id in einfo_info[mn]:
                if rel_id in self._process_extra_info[mn][pr_name]:
                    ...
                    rel_einfo = deepcopy(einfo_info[mn][rel_id])
                    for kw in ['esass_in_docker', 'working_dir']:
                        # This does return the popped value, but we don't care about it - the second argument tells
                        #  it to return None if the kw doesn't exist, but as long as it doesn't error we don't care
                        #  about that either
                        rel_einfo.pop(kw, None)

                    self._process_extra_info[mn][pr_name][rel_id] = rel_einfo
    ...
    def info(self):
        ...
        print(f"Beginning of earliest observation - {min([m.filtered_obs_info['start'].min() 
                                                          for m in self._missions.values()])}")
        print(f"End of latest observation - {max([m.filtered_obs_info['end'].max()
                                                  for m in self._missions.values()])}")
        for m in self._missions.values():
            print('')
            print(f'-- {m.pretty_name} --')
            print(f'   Internal DAXA name - {m.name}')
            print(f'   Chosen instruments - {', '.join(m.chosen_instruments)}')
            print(f'   Number of observations - {len(m)}')
            print(f'   Fully Processed - {m.processed}')
        print("-----------------------------------------------------\n")
